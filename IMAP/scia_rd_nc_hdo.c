...
.IDENTifer   SCIA_RD_NC_HDO
.AUTHOR      R.M. van Hees
.KEYWORDS    SCIA IMAP HDO
.LANGUAGE    ANSI C
.PURPOSE     read IMAP-HDO product in ADAGUC format
.COMMENTS    contains SCIA_RD_NC_HDO_META and SCIA_RD_NC_HDO_REC
.ENVIRONment None
...
.PURPOSE     read header from IMAP-HDO product in netCDF-4 (ADAGUC standard)
.INPUT/OUTPUT
  call as   SCIA_RD_NC_HDO_META( ncid, &hdr );
     input:
...
-------------------------*/
void SCIA_RD_NC_HDO_META( int ncid, struct imap_hdr *hdr )
{
     const char prognm[] = "SCIA_RD_NC_HDO_META";
...
{
     const char prognm[] = "SCIA_RD_NC_HDO_REC";
