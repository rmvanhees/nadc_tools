...
.IDENTifer   NADC_TOSOMI_WR_SQL_META
.AUTHOR      R.M. van Hees
.KEYWORDS    TOSOMI SQL
.LANGUAGE    ANSI C
.PURPOSE     write meta data of KNMI Tosomi product to table "meta_tosomi"
.INPUT/OUTPUT
  call as   metaID = NADC_TOSOMI_WR_SQL_META( conn, hdr, tosomi );
     input:  
             PGconn *conn              : PostgreSQL connection handle
	     struct tosomi_hdr *hdr    : header of Tosomi product
	     struct tosomi_rec *tosomi : records with Tosomi data
...
int NADC_TOSOMI_WR_SQL_META( PGconn *conn, const struct tosomi_hdr *hdr )
{
     const char prognm[] = "NADC_TOSOMI_WR_SQL_META";

     PGresult *res;

     char prod_version[8], l1b_version[10], l1b_product[ENVI_FILENAME_SIZE];
 ...
 */
     res = PQexec( conn,
                   "SELECT nextval(\'meta_tosomi_pk_meta_seq\')" );
     if ( PQresultStatus( res ) != PGRES_TUPLES_OK )
          NADC_GOTO_ERROR( prognm, NADC_ERR_SQL, PQresultErrorMessage(res) );
