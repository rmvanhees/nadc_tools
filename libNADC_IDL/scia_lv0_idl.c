...
unsigned int IDL_STDCALL _SCIA_LV0_RD_DET (int argc, void *argv[])
{
     ...
     num_chan = *(unsigned int *) argv[0];
     channels = (unsigned char *) argv[1];
     info  = (struct mds0_info *) argv[2];
     num_det   = *(unsigned int *) argv[3];
     det   = (struct IDL_mds0_det *) argv[4];
     data  = (unsigned int *) argv[5];

     /* set channel mask */
     nadc_set_param_chan(channels, (int) num_chan);
     ...
     for (nr = 0; nr < num_det; nr++) {
	  ...
	  SCIA_LV0_RD_DET(fd_nadc, info+nr, 1, &C_det);
