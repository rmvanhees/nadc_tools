...
;-
PRO SCIA_LV1_RD_EKD, dsd, ekd, status=status
...
; get index to data set descriptor 
  indx_dsd = WHERE( STRCOMPRESS(STRING(dsd[*].name), /REMOVE) $
                    EQ 'ERRORS_ON_KEY_DATA', count )
  IF count EQ 0 THEN $
     MESSAGE, ' FATAL, could not find keyword: ' + 'ERRORS_ON_KEY_DATA'
...
; check error status
  ...
  ENDIF ELSE $
     MESSAGE, ' no ERRORS_ON_KEY_DATA DSD records found', /INFORM
