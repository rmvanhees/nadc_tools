...
;-
PRO SCIA_LV1_RD_SCP, dsd, scp, status=status
...
; get index to data set descriptor 
  indx_dsd = WHERE( STRCOMPRESS(STRING(dsd[*].name), /REMOVE) $
                    EQ 'SPECTRAL_CALIBRATION', count )
  IF count EQ 0 THEN $
     MESSAGE, ' FATAL, could not find keyword: ' + 'SPECTRAL_CALIBRATION'
...
; check error status
  ...
  ENDIF ELSE $
     MESSAGE, ' no SPECTRAL_CALIBRATION DSD records found', /INFORM
