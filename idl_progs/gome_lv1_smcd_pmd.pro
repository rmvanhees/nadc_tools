...
;---------------------------------------------------------------------------
PRO GOME_LV1_SMCD_PMD, fsr, smcd, status=status, $
                       calibration=calibration, posit=posit
  compile_opt idl2,logical_predicate,hidden

; check required parameters
  IF N_PARAMS() NE 2 THEN BEGIN
     MESSAGE, ' Usage: GOME_LV1_SMCD_PMD, fsr, smcd, status=status, ',  $
...
; selection on relative index
  IF FIX(N_ELEMENTS( smcd )) EQ 0 THEN return
  indx_smcd = WHERE( smcd.indx_bands[0] NE (-1), nr_smcd )
