...
;-
PRO SCIA_OL2_RD_LGEO, dsd, lgeo, status=status
...
; get index to data set descriptor 
  indx_dsd = WHERE( STRCOMPRESS(STRING(dsd[*].name), /REMOVE) $
                    EQ 'GEOLOCATION_LIMB', count )
  IF count EQ 0 THEN $
     MESSAGE, ' FATAL, could not find keyword: ' + 'GEOLOCATION_LIMB'

;read Data Set Descriptor records
  num_dsd = ULONG( SIZE( dsd, /N_ELEMENTS ))
  num_geo = dsd[indx_dsd].num_dsr
  IF num_geo GT 0 THEN BEGIN
     lgeo = replicate( {lgeo_scia}, num_geo )
     num = call_external( lib_name('libnadc_idl'), '_SCIA_OL2_RD_LGEO', $
                          num_dsd, dsd, lgeo, /CDECL )

; check error status
     IF num NE num_geo THEN status = -1
  ENDIF ELSE $
     MESSAGE, ' no Limb/Occultation Geolocation records found', /INFORM
