...
;
;-
PRO SCIA_LV1_RD_SRSN, dsd, srsn, status=status
...
; get index to data set descriptor 
  indx_dsd = WHERE( STRCOMPRESS(STRING(dsd[*].name), /REMOVE) $
                    EQ 'NEW_SUN_REFERENCE', count )
  IF count EQ 0 THEN $
     MESSAGE, ' FATAL, could not find keyword: ' + 'NEW_SUN_REFERENCE'

;read Data Set Descriptor records
  num_dsd = ULONG( SIZE( dsd, /N_ELEMENTS ))
  num_srsn = dsd[indx_dsd].num_dsr
  IF num_srsn GT 0 THEN BEGIN
     srsn = replicate( {srsn_scia}, num_srsn )
     num = call_external( lib_name('libIDL_NADC'), '_SCIA_LV1_RD_SRSN', $
                          num_dsd, dsd, srsn, /CDECL )
; check error status
     IF num NE num_srsn THEN status = -1
  ENDIF ELSE $
     MESSAGE, ' no SRSN records found', /INFORM
