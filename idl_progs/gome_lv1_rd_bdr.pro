...
;---------------------------------------------------------------------------
PRO GOME_LV1_RD_BDR, nband, fsr, pcd, rec, status=status, $
                     calibration=calibration, posit=posit
  compile_opt idl2,logical_predicate,hidden

; check required parameters
  IF N_PARAMS() NE 4 THEN BEGIN
     MESSAGE, ' Usage: nband, fsr, pcd, rec, status=status, ',  $
              'calibration=calibration, posit=posit'
     status = -1
     RETURN
  ENDIF
...
; selection on relative index
  indx_pcd = WHERE( pcd.indx_bands[nband] NE (-1), nr_pcd )
  IF N_ELEMENTS( posit ) GT 0 THEN BEGIN
     IF posit[0] LT 0 OR posit[0] GE nr_pcd THEN BEGIN
        MESSAGE, 'Attempt to use subscript POSIT out of range.', /INFO
        status = -1
        RETURN
     ENDIF
     indx_pcd = pcd[posit[0]].indx_bands[nband]
     IF indx_pcd EQ (-1) THEN BEGIN
