...
.IDENTifer   ADAGUC_TOGOMI
.AUTHOR      R.M. van Hees
.KEYWORDS    GOME TOGOMI
.LANGUAGE    ANSI C
.PURPOSE     convert/combine TOGOMI product to ADAGUC standard
.INPUT/OUTPUT
  call as   
            adaguc_togomi [options] <input-files>
...
.INPUT/OUTPUT
  call as   HPSORT( dim, jday, rec );
    ...
    output:
	    ...
	    struct togomi_rec *rec :    array of records to be sorted as jday
...
-------------------------*/
static
void HPSORT( unsigned int dim, double *jday, struct togomi_rec *rec )
{
     register unsigned int ii,  ir, jj, ll;

     double jbuff;
     struct togomi_rec buff;

     const size_t sizeof_togomi_rec = sizeof(struct togomi_rec);
/*
 * initialization
 */
     jday--; rec--;
 ...
 */
	  ...
	  while ( jj <= ir ) {
	       ...
	       if ( jbuff < jday[jj] ) {                     /* demote jbuff */
		    jday[ii] = jday[jj];
		    (void) memcpy( rec+ii, rec+jj, sizeof_togomi_rec );
		    ii = jj;
		    jj <<= 1;
	       } else
	  ...
	  (void) memcpy( rec+ii, &buff, sizeof_togomi_rec );
     }
}
 ...
 */
     hdr.numProd = 0;
     do {
	  struct togomi_hdr hdr_tmp;
	  struct togomi_rec *rec_tmp = NULL;
...
/*
 * sort the TOGOMI records in time
 */
     if ( hdr.numRec > 1 ) {
	  ...
	  if ( param.flag_clip == PARAM_SET ) {
	       ...
	       if ( ihigh <= ilow ) {
		    ...
		    (void) memmove( rec, rec+ilow, 
				    hdr.numRec * sizeof(struct togomi_rec) );
	       }
	  }
	  free( jday );
     }
 ...
 * first, recontruct validity period from TOGOMI records
 */
