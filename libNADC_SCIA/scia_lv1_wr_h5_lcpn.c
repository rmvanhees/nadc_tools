...
.INPUT/OUTPUT
  call as    SCIA_LV1_WR_H5_LCPN(nr_lcpn, lcpn);
     input:  
...
void SCIA_LV1_WR_H5_LCPN(unsigned int nr_lcpn, const struct lcpn_scia *lcpn)
{
     hid_t   fid, ads_id;
     hsize_t adim;
     herr_t  stat;
 ...
 */
     adim = SCIENCE_PIXELS;
     lcpn_type[0] = H5Topen(fid, "mjd", H5P_DEFAULT);
     lcpn_type[1] = H5Topen(fid, "mjd", H5P_DEFAULT);
