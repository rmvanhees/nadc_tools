...
.INPUT/OUTPUT
  call as   nr_mds = SCIA_LV0_SELECT_MDS(source, nr_states, states, 
                                         &states_out);
     input: 
...
-------------------------*/
static
size_t SCIA_LV0_SELECT_MDS_PERIOD(const struct mds0_states *states, 
...
.INPUT/OUTPUT
  call as   nr_indx = SCIA_LV0_SELECT_MDS_STATE(states, nr_indx, indx);
     input:  
...
-------------------------*/
static
size_t SCIA_LV0_SELECT_MDS_STATE(const struct mds0_states *states, 
				 size_t nr_indx, size_t *indx)
       /*@modifies indx@*/
{
     register size_t ni;
     ...
     for (ni = 0; ni < nr_indx; ni++) {
	  if (nadc_get_param_state(states[indx[ni]].state_id))
	       indx[nr++] = indx[ni];     
     }
     return nr;
}
...
-------------------------*/
static
size_t SCIA_LV0_SELECT_MDS_COMPLETE(const struct mds0_states *states, 
				    size_t nr_indx, size_t *indx)
       /*@modifies indx@*/
{
     register size_t ni;
     ...
     for (ni = 0; ni < nr_indx; ni++) {
	  ...
	  if (nadc_get_param_uint8("write_aux") == PARAM_SET) {
	  ...
	  if (nadc_get_param_uint8("write_det") == PARAM_SET) {
...
-------------------------*/
static
size_t SCIA_LV0_SELECT_MDS_CRC(const struct mds0_states *states, 
			       size_t nr_indx, size_t *indx)
       /*@modifies indx@*/
{
     register size_t ns;
...
// 
//
size_t SCIA_LV0_SELECT_MDS(size_t num_states, const struct mds0_states *states,
 ...
 */
	 ...
	 && nadc_get_param_uint8("write_det") == PARAM_UNSET
	 && nadc_get_param_uint8("write_pmd") == PARAM_UNSET)
	  return 0u;
/*
 ...
 */
     if (nadc_get_param_uint8("flag_period") != PARAM_UNSET) {
	  nr_indx = SCIA_LV0_SELECT_MDS_PERIOD(states, nr_indx, indx_states);
	  if (nr_indx == 0) goto done;
     }
/*
 * apply State ID criterium
 */
     nr_indx = SCIA_LV0_SELECT_MDS_STATE(states, nr_indx, indx_states);
     if (nr_indx == 0) goto done;
/*
 * reject incomplete states
 */
     if (nadc_get_param_uint8("qcheck") == PARAM_SET) {
	  nr_indx = SCIA_LV0_SELECT_MDS_COMPLETE(states, nr_indx, indx_states);
	  if (nr_indx == 0) goto done;
     }
/*
 * apply CRC criterium 
 */
     if (nadc_get_param_uint8("qcheck") == PARAM_SET) {
	  nr_indx = SCIA_LV0_SELECT_MDS_CRC(states, nr_indx, indx_states);
	  if (nr_indx == 0) goto done;
     }
/*
