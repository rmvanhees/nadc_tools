...
.INPUT/OUTPUT
  call as   nr_dsr = SCIA_LV1C_UPDATE_CALOPT(do_init, &calopt);
     input:  
...
.COMMENTS    none
-------------------------*/
void SCIA_LV1C_UPDATE_CALOPT(int is_scia_lv1c, struct cal_options *calopt)
{
     ...
     if (is_scia_lv1c) {                        /* update calopt-structure */
	  if (nadc_get_param_uint8("flag_geoloc") == PARAM_SET) {
	       ...
	       calopt->start_lat = (int) (1e6 * rbuff[0]);
	       calopt->end_lat = (int) (1e6 * rbuff[1]);
	       nadc_get_param_range("longitude", rbuff);
	       calopt->start_lon = (int) (1e6 * rbuff[0]);
	       calopt->end_lon = (int) (1e6 * rbuff[1]);
	  }
	  ...
	  char *cpntr = nadc_get_param_string("infile");

	  const unsigned int calib_scia = nadc_get_param_uint32("calib_scia");

	  if (strrchr(cpntr, '/') != NULL)
	       (void) nadc_strlcpy(calopt->l1b_prod_name,
				   strrchr(cpntr, '/')+1, ENVI_FILENAME_SIZE);
	  else
	       (void) nadc_strlcpy(calopt->l1b_prod_name,
				   cpntr, ENVI_FILENAME_SIZE);
	  free(cpntr);
	  ...
	  if ((calib_scia & (DO_CORR_VIS_MEM|DO_CORR_IR_NLIN)) 
	       != UINT_ZERO)
	       calopt->mem_effect_cal = (signed char) -1;
	  else
	       calopt->mem_effect_cal = SCHAR_ZERO;
	  if ((calib_scia & (DO_CORR_AO|DO_CORR_DARK|DO_CORR_VDARK)) 
	       != UINT_ZERO)
