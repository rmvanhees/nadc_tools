...
.IDENTifer   SCIA_LV2_RD_SQADS
.AUTHOR      R.M. van Hees
.KEYWORDS    SCIA level 2 data
.LANGUAGE    ANSI C
.PURPOSE     read Summary of Quality Flags per State
.INPUT/OUTPUT
  call as   nr_dsr = SCIA_LV2_RD_SQADS( fd, num_dsd, dsd, &sqads );
     input:
...
static
void Sun2Intel_SQADS( struct sqads2_scia *sqads )
{
...
unsigned 
int SCIA_LV2_RD_SQADS( FILE *fd, unsigned int num_dsd, 
		       const struct dsd_envi *dsd, 
		       struct sqads2_scia **sqads_out )
{
     const char prognm[]   = "SCIA_LV2_RD_SQADS";
 ...
 */
     ...
     if ( ! Use_Extern_Alloc ) {
	  sqads_out[0] = (struct sqads2_scia *) 
	       malloc( dsd[indx_dsd].num_dsr * sizeof(struct sqads2_scia));
     }
 ...
 */
	  ...
	  (void) memcpy( &sqads->flag_pqf, sqads_pntr, 
			   NL2_SQADS_PQF_FLAGS * ENVI_UCHAR );
	  sqads_pntr += NL2_SQADS_PQF_FLAGS * ENVI_UCHAR;
/*
