...
.INPUT/OUTPUT
  call as   SCIA_LV0_WR_ASCII_INFO(num_states, states);
     input: 
...
void SCIA_LV0_WR_ASCII_INFO(size_t num_states, const struct mds0_states *states)
{
     ...
     if ((outfl = CRE_ASCII_File(cpntr, "info")) == NULL || IS_ERR_STAT_FATAL)
 ...
 */
     ...
     nadc_write_header(outfl, 0, cpntr, "Info records of a Level 0 Product");
     free(cpntr);
