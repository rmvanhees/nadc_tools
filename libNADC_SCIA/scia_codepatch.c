...
.COMMENTS    Contains scia_set_patch, scia_get_patch, scia_show_patch
.ENVIRONment None
...
.INPUT/OUTPUT
   call as:  scia_set_patch(patch_str);
     input:
...
.COMMENTS    none
-------------------------*/
void scia_set_patch(const char patch_str[])
{
     register short ns;
     register int   nr = 0;
 ...
 */
     if (strlen(patch_str) == 0 || strncmp(patch_str, "none", 4) == 0){
	  (void) nadc_set_param_uint16("patch_mask", SCIA_PATCH_NONE);
	  return;
     }
     if (strncmp(patch_str, "all", 3) == 0) {
	  (void) nadc_set_param_uint16("patch_mask", SCIA_PATCH_ALL);
	  return;
     }
/*
 ...
 */
     ...
     (void) nadc_set_param_uint16("patch_mask", patch_mask);
}
...
.INPUT/OUTPUT
   call as:  scia_get_patch(patch_str);
    output:  
            char patch_str[] :           string with encoded patch steps
...
.COMMENTS    none
-------------------------*/
void scia_get_patch(char patch_str[])
{
     register short ns;
     register int   nr;

     unsigned short patch_mask = nadc_get_param_uint16("patch_mask");
     
     if (patch_mask == SCIA_PATCH_NONE) {
	  (void) strcpy(patch_str, "none");
	  return;
     }
     if (patch_mask == SCIA_PATCH_ALL) {
	  (void) strcpy(patch_str, "all");
	  return;
     }
