...
.INPUT/OUTPUT
  call as   SCIA_OL2_WR_ASCII_CLD(num_dsr, cld);
     input:
...
.COMMENTS    None
-------------------------*/
void SCIA_OL2_WR_ASCII_CLD(unsigned int num_dsr, const struct cld_sci_ol *cld)
{
     ...
     if ((outfl = CRE_ASCII_File(cpntr, "cld")) == NULL || IS_ERR_STAT_FATAL)
 ...
 */
     cpntr = nadc_get_param_string("outfile");
     nadc_write_header(outfl, 0, cpntr, "Clouds and Aerosol Data set(s)");
     free(cpntr);
...
.INPUT/OUTPUT
  call as   SCIA_OL2_WR_ASCII_NFIT(mds_name, num_dsr, nfit);
     input:
...
void SCIA_OL2_WR_ASCII_NFIT(const char *mds_name, 
			    unsigned int num_dsr, const struct nfit_scia *nfit)
{
     register unsigned int nd, nr;
     ...
     cpntr = nadc_get_param_string("outfile");
     if ((outfl = CRE_ASCII_File(cpntr, mds_name)) == NULL || IS_ERR_STAT_FATAL)
	  NADC_RETURN_ERROR(NADC_ERR_FILE_CRE, cpntr);
     free(cpntr);
     cpntr = nadc_get_param_string("infile");
     nadc_write_header(outfl, 0, cpntr, mds_name);
     free(cpntr);
/*
...
.INPUT/OUTPUT
  call as   SCIA_OL2_WR_ASCII_LFIT(mds_name, num_dsr, lfit);
     input:
...
void SCIA_OL2_WR_ASCII_LFIT(const char *mds_name, 
			     unsigned int num_dsr, const struct lfit_scia *lfit)
{
...
.INPUT/OUTPUT
  call as   SCIA_OL2_WR_ASCII_LCLD(num_dsr, lcld);
     input:
	    unsigned int num_dsr      : number of data sets
...
.COMMENTS    None
-------------------------*/
void SCIA_OL2_WR_ASCII_LCLD(unsigned int num_dsr, 
			    const struct lcld_scia *lcld)
{
     ...
     cpntr = nadc_get_param_string("outfile");
     if ((outfl = CRE_ASCII_File(cpntr, "lcld")) == NULL || IS_ERR_STAT_FATAL)
	  NADC_RETURN_ERROR(NADC_ERR_FILE_CRE, cpntr);
     free(cpntr);
/*
 ...
 */
     cpntr = nadc_get_param_string("infile");
     nadc_write_header(outfl, 0, cpntr, "Limb Clouds Data set(s)");
     free(cpntr);
     for (nd = 0; nd < num_dsr; nd++) {
	  nr = 0;
