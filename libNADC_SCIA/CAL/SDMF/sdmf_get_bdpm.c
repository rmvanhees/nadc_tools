...
{
     const char prognm[] = "SDMF_get_BDPM_24";
 ...
 */
     if ( (db_fp = fopen( bdpm_fl, "r" )) == NULL )
	  NADC_GOTO_ERROR( prognm, NADC_ERR_FILE, bdpm_fl );
...
{
     const char prognm[] = "sdmf_get_bdpm";
 ...
 */
     ...
     if ( IS_ERR_STAT_FATAL )
	  NADC_GOTO_ERROR( prognm, NADC_ERR_FATAL, "SDMF_get_BDPM_24" );
     if ( ! fnd_24 ) (void) fprintf( stderr, "# no solution for SDMF v2.4\n" );
     fnd_30 = SDMF_get_BDPM_30( orbit, bdpm_30 );
     if ( IS_ERR_STAT_FATAL )
	  NADC_GOTO_ERROR(prognm, NADC_ERR_FATAL, "SDMF_get_BDPM_30");
