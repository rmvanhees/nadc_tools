...
-------------------------*/
static
void SCIA_LV1_INIT_DSD(unsigned int num_dsd_in, 
...
.INPUT/OUTPUT
  call as   SCIA_LV1_WR_DSD_INIT(fp_out, num_dsd_in, dsd_in);
     input:
...
.COMMENTS    none
-------------------------*/
void SCIA_LV1_WR_DSD_INIT(FILE *fp_out, 
 ...
 */
     SCIA_LV1_INIT_DSD(num_dsd_in, dsd_in);
/*
...
.INPUT/OUTPUT
  call as   SCIA_LV1_SET_NUM_ATTACH(fp_in, num_dsd_in, dsd_in);
     input:
	    FILE *fp_in               :  (open) stream pointer (input)
	    unsigned int num_dsd_in   :  number of DSD's (input)
	    struct dsd_envi *dsd_in   :  DSD's records (input)
...
.COMMENTS    none
-------------------------*/
void SCIA_LV1_SET_NUM_ATTACH(FILE *fp_in, 
			     unsigned int num_dsd_in, 
			     const struct dsd_envi *dsd_in)
 ...
 */
     ...
     for (ni = 0; ni < num_dsr; ni++) {
	  ...
	  if (state[ni].flag_mds != UCHAR_ZERO) {
	       found = FALSE;
	  } else if (nadc_get_param_uint8("write_lv1c") == PARAM_UNSET) {
