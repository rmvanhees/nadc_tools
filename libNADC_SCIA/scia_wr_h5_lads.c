...
.INPUT/OUTPUT
  call as    SCIA_WR_H5_LADS(nr_lads, lads);
     input:  
...
unsigned int SCIA_RD_H5_LADS(struct lads_scia *lads)
...
void SCIA_WR_H5_LADS(unsigned int nr_lads, const struct lads_scia *lads)
     /*@globals lads_size, lads_offs@*/
{
     hid_t   fid, ads_id;
     hsize_t adim;
 ...
 */
     temp_type00 = H5Topen(fid, "mjd", H5P_DEFAULT);
     adim = NUM_CORNERS;
