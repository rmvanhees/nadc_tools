...
.INPUT/OUTPUT
  call as    SCIA_OL2_WR_H5_NGEO(nr_geo, geo);
             SCIA_OL2_WR_H5_LGEO(nr_geo, geo);
     input:  
...
void SCIA_OL2_WR_H5_NGEO(unsigned int nr_ngeo, const struct ngeo_scia *ngeo)
{
     hid_t   fid, ads_id;
     hid_t   type_id;
     hbool_t compress;
 ...
 */
     ngeo_type[0] = H5Topen(fid, "mjd", H5P_DEFAULT);
     ...
     ngeo_type[8] = H5Topen(fid, "coord", H5P_DEFAULT);
     adim = NUM_CORNERS;
     type_id = H5Topen(fid, "coord", H5P_DEFAULT);
     ngeo_type[9] = H5Tarray_create(type_id, 1, &adim);
     (void) H5Tclose(type_id);
     ngeo_type[10] = H5Topen(fid, "coord", H5P_DEFAULT);
/*
 * create table
 */
...
void SCIA_OL2_WR_H5_LGEO(unsigned int nr_lgeo, const struct lgeo_scia *lgeo)
{
     hid_t   fid, ads_id;
     hid_t   type_id;
     hbool_t compress;
 ...
 * define user-defined data types of the Table-fields
 */
     lgeo_type[0] = H5Topen(fid, "mjd", H5P_DEFAULT);
     ...
     lgeo_type[9] = H5Topen(fid, "coord", H5P_DEFAULT);
     type_id = H5Topen(fid, "coord", H5P_DEFAULT);
