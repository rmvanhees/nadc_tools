...
.RETURNS     return TRUE if both files are equal
.COMMENTS    None
.ENVIRONment None
...
bool nadc_file_equal( const char *fl_name_1, const char *fl_name_2 )
{
     struct stat buf1, buf2;
/*
 * get file status of the first file
 */
     if ( stat( fl_name_1, &buf1 ) != 0 ) return FALSE;
/*
 * get file status of the second file
 */
     if ( stat( fl_name_2, &buf2 ) != 0 ) return FALSE;
