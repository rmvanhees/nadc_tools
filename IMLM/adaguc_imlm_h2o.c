...
.IDENTifer   ADAGUC_IMLM_H2O
.AUTHOR      R.M. van Hees
.KEYWORDS    SRON IMLM Sciamachy
.LANGUAGE    ANSI C
.PURPOSE     convert/combine IMLM H2O product to ADAGUC standard
.INPUT/OUTPUT
  call as   
            adaguc_imlm_h2o [options] <input-files>

.RETURNS     non-negative on success, negative on failure
.COMMENTS    None
.ENVIRONment None
.VERSION     1.1     18-May-2011   fixed bug in selection algorithm, RvH
             1.0     12-Apr-2011   initial release by R. M. van Hees
------------------------------------------------------------*/
/*
...
unsigned int SELECT_IMLM_RECORDS( unsigned int numRec, struct imlm_rec *rec )
{
     ...
     for ( nr = 0u; nr < numRec; nr++ ) {
	  if ( rec_buff[nr].meta.bs == 0 && rec_buff[nr].meta.eflag == 0 ) {
	       (void) memcpy( rec+num, rec_buff+nr, sizeof(struct imlm_rec) );
	       num++;
	  }
     }
     free( rec_buff );
...
{
     const char prognm[] = "adaguc_imlm_h2o";
 ...
 */
     ...
     do {
	  ...
	  if ( H5Fis_hdf5( flname ) ) {
	       ...
	       SCIA_RD_NC_H2O_META( ncid, &hdr_tmp );
	       hdr_tmp.numRec = SCIA_RD_NC_H2O_REC( ncid, &rec_tmp );
 ...
 * first, recontruct validity period from IMLM-H2O records
 */
     ...
     SCIA_WR_NC_H2O_META( ncid, &hdr );
     if ( IS_ERR_STAT_FATAL )
	  NADC_GOTO_ERROR( prognm, NADC_ERR_HDF_WR, "IMLM-H2O header" );

     SCIA_WR_NC_H2O_REC( ncid, hdr.numRec, rec );
     if ( IS_ERR_STAT_FATAL )
	  NADC_GOTO_ERROR( prognm, NADC_ERR_HDF_WR, "IMLM-H2O records" );
