...
.IDENTifer   SCIA_WR_SQL_H2O_META
.AUTHOR      R.M. van Hees
.KEYWORDS    SRON IMLM H2O SQL
.LANGUAGE    ANSI C
.PURPOSE     write meta data of SRON IMLM product to table "meta_imlm_h2o"
.INPUT/OUTPUT
  call as   SCIA_WR_SQL_H2O_META( conn, hdr );
     input:  
...
void SCIA_WR_SQL_H2O_META( PGconn *conn, const struct imlm_hdr *hdr )
{
     const char prognm[] = "SCIA_WR_SQL_H2O_META";
 ...
 */
     res = PQexec( conn,
                   "SELECT nextval(\'meta_imlm_h2o_pk_meta_seq\')" );
     if ( PQresultStatus( res ) != PGRES_TUPLES_OK )
          NADC_GOTO_ERROR( prognm, NADC_ERR_SQL, PQresultErrorMessage(res) );
