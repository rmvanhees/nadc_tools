...
.IDENTifer   SCIA_WR_SQL_CO_TILE
.AUTHOR      R.M. van Hees
.KEYWORDS    SCIA IMLM CO SQL
.LANGUAGE    ANSI C
.PURPOSE     write tile information of a Sciamachy IMLM product to database
.INPUT/OUTPUT
  call as   SCIA_WR_SQL_CO_TILE( conn, sciafl, num_rec, rec );
     input:  
...
#define TILE_TBL_NAME "tile_imlm_co"

#define SQL_INSERT_TILE \
"INSERT INTO %s (pk_tile,fk_meta,julianDay,integrationTime,errorFlag,\
 meanElevation,cloudFraction,surfaceAlbedo,VCD_CO,VCD_CO_ERROR,VCD_CH4,\
 VCD_CH4_ERROR,tile) \
...
"INSERT INTO %s (pk_tile,fk_meta,julianDay,integrationTime,errorFlag,\
 meanElevation,cloudFraction,surfaceAlbedo,VCD_CO,VCD_CO_ERROR,tile)\
...
void SCIA_WR_SQL_CO_TILE( PGconn *conn, const char *prodName,
			    unsigned int num_rec, 
			    const struct imlm_rec *rec )
{
     const char prognm[] = "SCIA_WR_SQL_CO_TILE";
 ...
 */
     for ( nr = 0; nr < num_rec; nr++ ) {
	  ...
	  res = PQexec( conn,
			"SELECT nextval(\'tile_imlm_co_pk_tile_seq\')" );
	  if ( PQresultStatus( res ) != PGRES_TUPLES_OK )
