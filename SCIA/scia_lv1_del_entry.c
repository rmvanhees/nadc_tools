...
.IDENTifer   SCIA_LV1_DEL_ENTRY
.AUTHOR      R.M. van Hees
.KEYWORDS    SCIA SQL
.LANGUAGE    ANSI C
.PURPOSE     remove entries in database for a Sciamachy level 1b product
.INPUT/OUTPUT
  call as   SCIA_LV1_DEL_ENTRY(conn, flname);
     input:  
             PGconn *conn        :  PostgreSQL connection handle
             char *flname        :  filename of the Sciamachy Level 1b product

.RETURNS     Nothing, error status passed by global variable ``nadc_stat''
.COMMENTS    Removes entries related to file "flname" from tables:
             meta__1P, stateinfo_meta__1P and reverse stateinfo
.ENVIRONment None
.VERSION     2.1     04-Oct-2012   added verbose flag, RvH
             2.0     08-Jun-2007   port to PostgreSQL R. M. van Hees
...
"SELECT unnest(fk_stateinfo) FROM stateinfo_meta__1P WHERE\
 fk_meta=(SELECT pk_meta FROM meta__1P where name=\'%s\')"

#define DELETE_FROM_META \
"DELETE FROM meta__1P WHERE name=\'%s\'"

#define UPDATE_STATEINFO \
"UPDATE stateinfo \
SET softVersion=SUBSTR(softVersion,1,1) || \'0\' || SUBSTR(softVersion,3,1) \
WHERE pk_stateinfo=%u AND SUBSTR(softVersion,2,1)=\'%s\'"
...
void SCIA_LV1_DEL_ENTRY(PGconn *conn, const char *flname)
{
     register int nr;
